...
std::vector<std::string> SortedNiceNames(const std::unordered_set<Component*>& comps)
{
	...
	std::transform(comps.begin(), comps.end(), ret.begin(), [](const Component* comp) -> std::string { return comp->NiceName('.'); });
	std::sort(ret.begin(), ret.end());

	return ret;
}

Component& AddComponentDefinition(std::unordered_map<std::string, Component*>& components, const filesystem::path& path)
{
