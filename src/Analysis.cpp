...
static void StrongConnect(std::vector<ComponentLakos*>& stack, size_t& index, ComponentLakos* c)
{
	...
	if (c->lowlink == c->index)
	{
		...
		do
		{
			ComponentLakos* comp = stack.back();
			stack.pop_back();
...
{
	std::vector<ComponentLakos*> stack;
	size_t						 index = 1;
...
{
	ComponentLakos* target = components[str];
	if (target)
	{
		for (auto& c : components)
		{
...
void PropagateExternalIncludesLakos(std::unordered_map<std::string, File>& files)
{
	...
	do
	{
		...
		for (auto& fp : files)
		{
			if (fp.second.hasExternalInclude && fp.second.lakosComponent)
			{
				for (auto& dep : fp.second.dependencies)
				{
					if (!dep->hasExternalInclude && dep->lakosComponent == fp.second.lakosComponent)
					{
...
void MapIncludesToDependencies(std::unordered_map<std::string, std::string>& includeLookup, std::map<std::string, std::vector<std::string>>& ambiguous,
							   std::unordered_map<std::string, Component*>& components, std::unordered_map<std::string, File>& files)
{
	for (auto& fp : files)
	{
