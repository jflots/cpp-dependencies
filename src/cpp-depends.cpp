...
class Operations
{
	...
	void RunCommands()
	{
		...
		while (it != end)
		{
			...
			localEnd++;
			while (localEnd != end && ((*localEnd)[0] != '-' || (*localEnd)[1] != '-'))
				localEnd++;
	...
	{
		if (!withLoc && loadStatus >= FastLoad)
			return;
		if (withLoc && loadStatus >= FullLoad)
			return;
		//LoadFileList(components, files, projectRoot, inferredComponents, withLoc);
		LoadMultipleFileList(components, files, projectDirectories, inferredComponents, withLoc);
		...
		// Lakos part
		LakosComponent();
		ExtractPublicDependencies(mapLakosComponents);
		FindCircularDependencies(mapLakosComponents);
	...
	void Dir(std::vector<std::string> args)
	{
		...
		else
		{
			...
			for (auto& s : args)
			{
				projectDirectories.push_back(s);
			}
		}
		UnloadProject();
	}
	...
	void Ignore(std::vector<std::string> args)
	{
		...
		for (auto& s : args)
			Configuration::Get().blacklist.push_back(s);
		UnloadProject();
	}
	...
	void Graph(std::vector<std::string> args)
	{
		...
		else
		{
			...
			// Lakos part
			OutputFlatDependencies(mapLakosComponents, args[0]);
		}
	}
	void GraphCycles(std::vector<std::string> args)
	{
		...
		else
		{
			...
			// Lakos part
			OutputCircularDependencies(mapLakosComponents, args[0]);
		}
	}
	void GraphTarget(std::vector<std::string> args)
	{
		...
		else
		{
			...
			// Lakos part
			PrintGraphOnTarget(args[1], mapLakosComponents[args[0]]);
		}
	}
	...
	void Cycles(std::vector<std::string> args)
	{
		...
		else
		{
			...
			// Lakos part
			PrintCyclesForTarget(mapLakosComponents[args[0]]);
		}
	}
	...
	void Stats(std::vector<std::string>)
	{
		...
		std::cout << components.size() << " components with " << totalPublicLinks << " public dependencies, " << totalPrivateLinks << " private dependencies\n";
		...
		std::set<std::string> publicDepStr;
		std::set<std::string> privateDepStr;
		totalPublicLinks = 0, totalPrivateLinks = 0;
		for (const auto& c : mapLakosComponents)
		{
			totalPublicLinks += c.second->pubDeps.size();

			for (auto& aPubDep : c.second->pubDeps)
			{
				publicDepStr.insert(aPubDep->NiceName('.'));
			}

			totalPrivateLinks += c.second->privDeps.size();
			for (auto& aPrivDep : c.second->privDeps)
			{
				privateDepStr.insert(aPrivDep->NiceName('.'));
			}

		}
		std::cout << mapLakosComponents.size() << " components with " << totalPublicLinks << " public dependencies, " << totalPrivateLinks << " private dependencies\n";
	/*	std::cout << " public dependencies: \n";

		for (auto& str : publicDepStr)
		{
			std::cout << "  " << str << "\n";
		}

		std::cout << " private dependencies: \n";

		for (auto& str : privateDepStr)
		{
			std::cout << "  " << str << "\n";
		}*/
		std::cout << "Detected " << NodesWithCycles(mapLakosComponents) << " nodes in cycles\n";
	}
	...
	void InOut(std::vector<std::string> args)
	{
		...
		for (auto& s : args)
		{
			PrintLinksForTarget(mapLakosComponents[s]);
		}
	}
	void Shortest(std::vector<std::string> args)
	{
		...
		Component *from = components[targetFrom(args[0])], *to = components[targetFrom(args[1])];
		if (!from)
		{
		...
		ComponentLakos *fromLakos = mapLakosComponents[args[0]], *toLakos = mapLakosComponents[args[1]];
		if (!fromLakos)
		{
			std::cout << "No such component " << args[0] << "\n";
		}
		...
		{
			FindSpecificLink(files, fromLakos, toLakos);
		}
	}
	...
	void Info(std::vector<std::string> args)
	{
		...
		for (auto& s : args)
		{
			PrintInfoOnTarget(mapLakosComponents[s]);
		}
	}
	...
	void UsedBy(std::vector<std::string> args)
	{
		...
		std::vector<std::string> dependenciesList;
		for (auto& s : args)
		{
			std::cout << "File " << s << " is used by:\n";
			auto found = files.find("./" + s);
			if (found != files.end())
			{
				File* f = &found->second;
				for (auto& p : files)
				{
					...
					{
						dependenciesList.push_back(p.second.path.string());
		...
		std::sort(dependenciesList.begin(), dependenciesList.end());

		for (auto& dependentFile : dependenciesList)
		{
			std::cout << "  " << dependentFile << "\n";
		}
	}
	...
	void Outliers(std::vector<std::string>)
	{
		...
		/*	PrintAllComponents(components,
							   "Libraries with no links in:", [](const Component& c) { return c.type == "library" && !c.files.empty() && c.pubLinks.empty() && c.privLinks.empty(); });
			PrintAllComponents(components, "Libraries with too many outward links:", [](const Component& c) {
				return c.pubDeps.size() + c.privDeps.size() > Configuration::Get().componentLinkLimit;
			});
			PrintAllComponents(components,
							   "Libraries with too few lines of code:", [](const Component& c) { return !c.files.empty() && c.loc() < Configuration::Get().componentLocLowerLimit; });
		...
		FindCircularDependencies(mapLakosComponents);
		PrintAllComponents(mapLakosComponents, "Lakos components that are part of a cycle:", [](const ComponentLakos& c) { return !c.circulars.empty(); });
	...
	{
		std::vector<std::pair<File*, size_t>> allCountingFiles;

		LoadProject(true);
		std::unordered_map<File*, size_t> includeCount;
		for (auto& f : files)
		{
			if (f.second.hasInclude)
				continue;
			...
			while (!todo.empty())
			{
				...
				{
					if (filesIncluded.insert(d).second)
						todo.push_back(d);
				}
			}
		...
		for (auto& f : files)
		{
			...
			{
				std::pair<File*, size_t> pairFileSize(&f.second, f.second.includeCount * total);
				allCountingFiles.push_back(pairFileSize);
		...
		std::sort(allCountingFiles.begin(), allCountingFiles.end(),
				  [&](const std::pair<File*, size_t>& fileInfo1, const std::pair<File*, size_t>& fileInfo2) { return fileInfo1.second > fileInfo2.second; });

		// Let's output a CSV file
		std::cout << " Impact (Total LOC) ; File LOC ;  Times Included ; Path \n";

		for (const std::pair<File*, size_t>& pairInfo : allCountingFiles)
		{
			auto total = pairInfo.second;
			std::cout << total << ";" << total / pairInfo.first->includeCount << ";" << pairInfo.first->includeCount << ";" << pairInfo.first->path.string() << "\n";
		/*	std::cout << "impact " << total << " total LOC with " << total / pairInfo.first->includeCount << " LOC used " << pairInfo.first->includeCount << " times from "
					  << pairInfo.first->path.string() << "\n";*/
		}
	}
	...
	void Help(std::vector<std::string>)
	{
		...
		//std::cout << "  --lakoscomponent                 : Attempt to create components made of couples of corresponding header, source files.\n";
	}

	void LakosComponent()
	{
		// find all files with original code
	//	LoadProject(true);

		// reconcile pairs of files to create components
		// a component

		std::vector<const File*> allFiles;
		allFiles.reserve(files.size());
		for (const auto& aFile : files)
		{
			allFiles.push_back(&aFile.second);
		}

		std::sort(allFiles.begin(), allFiles.end(), [&](const File* file1, const File* file2) { return (file1->path.stem().string() < file2->path.stem().string()); });

#ifdef _DEBUG
/*	for (auto aFile : allFiles)
	{
		std::cout << aFile->path.string() << "\n";
	}*/
#endif
		// pour l'instant ignorer les ITF, de toute façon, pas de .cpp correspondant

		std::vector<ComponentLakos*> allLakosComponents;
		allLakosComponents.reserve(files.size());

		ComponentLakos* aComponent = nullptr;

		for (size_t iFile = 0; iFile < allFiles.size(); ++iFile)
		{
			aComponent = nullptr;

			if (iFile < allFiles.size() - 1 && allFiles[iFile]->path.stem() == allFiles[iFile + 1]->path.stem())
			{
				if (IsHeader(allFiles[iFile]->path.extension().string()))
				{
					aComponent = new ComponentLakos(allFiles[iFile], allFiles[iFile + 1]);
				}
				else // source file
				{
					aComponent = new ComponentLakos(allFiles[iFile + 1], allFiles[iFile]);
				}

				allLakosComponents.push_back(aComponent);
				mapLakosComponents[allFiles[iFile]->path.stem().string()] = aComponent;

				// on référence le fichier à un composant
				const_cast<File*>(allFiles[iFile])->lakosComponent	 = aComponent;
				const_cast<File*>(allFiles[iFile + 1])->lakosComponent = aComponent;

#ifdef _DEBUG
				std::cout << iFile << " component : " << allFiles[iFile]->path.stem() << ", " << allFiles[iFile + 1]->path.stem() << "\n";
#endif
				++iFile;
			}
			else // outliers
			{
				aComponent = new ComponentLakos(allFiles[iFile], nullptr);
				allLakosComponents.push_back(aComponent);
				mapLakosComponents[allFiles[iFile]->path.stem().string()] = aComponent;

				const_cast<File*>(allFiles[iFile])->lakosComponent = aComponent;
#ifdef _DEBUG
				std::cout << iFile << " outlier : " << allFiles[iFile]->path.stem() << " \n";
#endif
			}
		}

		for (auto& aLakosComponent : allLakosComponents)
		{
			// treat header and possibly source files of a component

			for (auto f : aLakosComponent->m_files)
			{
				auto aFile = const_cast<File*>(f);

				for (auto& p : aFile->rawIncludes)
				{
					...
					if (localFile)
					{
						...
						File* dep		= &files.find(tmp)->second;
						dep->hasInclude = true;
						aFile->dependencies.insert(dep);

						if (aFile->lakosComponent != dep->lakosComponent) // && aFile->lakosComponent && dep->lakosComponent)
						{
							aFile->lakosComponent->privDeps.insert(dep->lakosComponent);

							dep->lakosComponent->privLinks.insert(aFile->lakosComponent);
	...
	std::unordered_map<std::string, ComponentLakos*> mapLakosComponents;
	...
	std::vector<filesystem::path>					projectDirectories;
};
